...
const updateTeamStatistics = async (match:Match, homeTeamPoints: number, awayTeamPoints:number) => {
  ...
  // If the admin updates the matchup
    ...
    if (homeTeam && awayTeam) {
      ...
      homeTeam.pointScored = homeTeam.pointScored - prevHomePoints + homeTeamPoints;
      homeTeam.pointAgainst = homeTeam.pointAgainst - prevAwayPoints  + awayTeamPoints;
      homeTeam.diff = homeTeam.pointScored - homeTeam.pointAgainst;
  
      awayTeam.pointScored =  awayTeam.pointScored - prevAwayPoints  + awayTeamPoints;
      awayTeam.pointAgainst = awayTeam.pointAgainst - prevHomePoints  + homeTeamPoints;
      awayTeam.diff = awayTeam.pointScored - awayTeam.pointAgainst;
      await homeTeam.save();
      await awayTeam.save();
    }
